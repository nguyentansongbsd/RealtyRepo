...
namespace Action_Resv_GenPMS
{
    public class Action_Resv_GenPMS : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            try
            {
                ...
                Entity interate = service.Retrieve(((EntityReference)enPS["bsd_interestratemaster"]).LogicalName, ((EntityReference)enPS["bsd_interestratemaster"]).Id,
                        new ColumnSet(new string[] { "bsd_gracedays" }));
                ...
                if (listCreateIns != null && listCreateIns.Count > 0)
                {
                    for (int i = 0; i < listCreateIns.Count - 1; i++)
                    {
                            ...
                            && RetrieveLocalTimeFromUTCTime((DateTime)listCreateIns[i]["bsd_duedate"], service).Date < RetrieveLocalTimeFromUTCTime((DateTime)listCreateIns[i + 1]["bsd_duedate"], service).Date)
                        {
                            ...
                            throw new InvalidPluginExecutionException("Tiến độ thanh toán không hợp lệ. Vui lòng kiếm tra lại thông tin.");
                        }
                    }
                }
                #endregion
        ...
        private void GenPaymentScheme(ref Entity enQuote, Entity enPS, int type, decimal phiBaoTriPaid, ref List<Entity> listCreateIns, int graceday)
        {
            ...
            int orderNumber = 0;
            if (len == 0)
                throw new InvalidPluginExecutionException("No payment scheme detail in payment scheme '" + (string)enPS["bsd_name"] + "'. Please setup payment scheme again!");
            traceS.Trace("4.3");
            ...
            for (int i = 0; i < len; i++) // len = so luong INS detail
            {
                ...
                if (!listInsMaster.Entities[i].Contains("bsd_duedatecalculatingmethod"))
                    throw new InvalidPluginExecutionException("Please choose Duedate Calculating Method for '" + (string)listInsMaster.Entities[i]["bsd_name"] + "' of Payment Scheme " + (string)enPS["bsd_name"] + " on Master data!");
        ...
        ref int indexInsValueNull, ref decimal valuePer, ref List<Entity> listCreateIns, EntityCollection listInsMaster, EntityCollection wordTemplateList)
        {
            ...
            if (en.Contains("bsd_startingwith") && orderNumber != 1)
            {
                if (!en.Contains("bsd_nextperiodtype"))
                    throw new InvalidPluginExecutionException("Please select field 'Next period type' on '" + en["bsd_name"].ToString() + "' of Payment Scheme " + (string)enPS["bsd_name"] + " on Master data!");

                int type = ((OptionSetValue)en["bsd_nextperiodtype"]).Value;
                if (type == 1)//month
                {
                    if (!en.Attributes.Contains("bsd_numberofnextmonth"))
                        throw new InvalidPluginExecutionException("Please select field 'Number next month' on '" + en["bsd_name"].ToString() + "' of Payment Scheme " + (string)enPS["bsd_name"] + " on Master data!");
                }
                else if (type == 2)//day
                {
                    if (!en.Attributes.Contains("bsd_numberofnextdays"))
                        throw new InvalidPluginExecutionException("Please select field 'Number of next days' on '" + en["bsd_name"].ToString() + "' of Payment Scheme " + (string)enPS["bsd_name"] + " on Master data!");
                }
            }
            ...
            tmp["bsd_signcontractinstallment"] = en.Contains("bsd_signcontractinstallment") ? en["bsd_signcontractinstallment"] : false;
        ...
        private DateTime calculateDuedate(Entity enQuote, Entity enIns, List<Entity> listCreateIns, EntityCollection listInsMaster, bool isFirstIns)
        {
            ...
            if (isFirstIns)
            {
                if (!enQuote.Contains("bsd_ngaydatcoc"))
                    throw new InvalidPluginExecutionException("Reservation did not contain 'Quotation Confirmed Date'! Please check again!");
                ...
                if (enQuote.Contains("statuscode") && ((OptionSetValue)enQuote["statuscode"]).Value == 100000000) //reservation
                {
                    if (!enQuote.Contains("bsd_reservationtime"))
                        throw new InvalidPluginExecutionException("Reservation did not contain 'Reservation Time'! Please check again!");
            ...
            if (!flag)
            {
                ...
                else throw new InvalidPluginExecutionException("Tiến độ thanh toán không hợp lệ. Vui lòng kiếm tra lại thông tin.");
            }
        ...
        private DateTime get_EstimatehandoverDate(Entity enQuote)
        {
            ...
            <fetch top=""1"">
              <entity name=""bsd_product"">
                ...
                <link-entity name=""bsd_quote"" from=""bsd_unitno"" to=""bsd_productid"">
                  <filter>
                    <condition attribute=""bsd_quoteid"" operator=""eq"" value=""{enQuote.Id}"" />
                  </filter>
                </link-entity>
              </entity>
            </fetch>";
            ...
            Entity en = rs.Entities[0];
            if (!en.Contains("bsd_estimatehandoverdate"))
                d = get_EstimateFromProject(enQuote);
        ...
        private DateTime get_EstimateFromProject(Entity enQuote)
        {
            ...
            if (!enQuote.Contains("bsd_projectid"))
                throw new InvalidPluginExecutionException("Cannot find 'Project' in Quotation Reservation. Please check again!");
        ...
        public string TienBangChu(string sSoTienIn)
        {
            string am = "";
            ...
            return am + temp + chuoi.Substring(1, chuoi.Length - 2) + " đồng";
            ...
            if (rs != null && rs.Entities != null && rs.Entities.Count > 0)
            {
                ...
                foreach (var item in rs.Entities)
                {
                    ...
                    if (isTypePercent && isTypeAmount)
                        throw new InvalidPluginExecutionException("Các đợt không được có đồng thời tiền và phần trăm. Vui lòng kiểm tra lại.");
                ...
                if ((fullAmount && amountDiscountTmp < sumValueNotNull) || (isTypeAmount && amountDiscountTmp < sumValueNotNull))
                    throw new InvalidPluginExecutionException("Tổng giá trị đợt đang vượt quá tổng giá trị HĐ. Vui lòng kiểm tra lại.");

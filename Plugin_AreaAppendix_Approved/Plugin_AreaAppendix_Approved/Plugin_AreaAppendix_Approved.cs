...
namespace Plugin_AreaAppendix_Approved
{
    public class Plugin_AreaAppendix_Approved : IPlugin
    {
        ...
        void IPlugin.Execute(IServiceProvider serviceProvider)
        {
            try
            {
                ...
                traceS.Trace("2");
                int status = enTarget.Contains("statuscode") ? ((OptionSetValue)enTarget["statuscode"]).Value : -99;
                if (status != 100000002)    //Approved
                    return;
